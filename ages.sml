...
sig
    ...
    val toLnForm: Form.form -> (Ln.atom list * Ln.atom) list
...
fun ages input =
    let val normalizedInput = F.toHorn (List.map (fn p => (F.nnf o F.toQF o F.skolem) p) input)
