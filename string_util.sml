...
sig 
    ...
    val find: string -> char -> int option
...
fun find str ch =
    let fun loop (i,"") = NONE
	  | loop (i,s) = if String.size s > 0 andalso String.sub (s,0) = ch
			 then SOME i
			 else loop (i+1,String.substring (s,1,String.size s-1))
    in loop (0,str)
    end

fun concat [] = ""
  | concat (str::rest) = str ^ (concat rest)

fun split str ch =
    let fun loop ("",ss) _ = rev ss
	  | loop (str,ss) ch = case find str ch of
				   SOME pos => let val head = String.substring (str,0,pos)
						   val rest = String.substring (str,pos+1,String.size str-pos-1)
						in loop (rest,head::ss) ch
						end 
				  | NONE => loop ("",str::ss) ch
    in loop (str,[]) ch
    end
